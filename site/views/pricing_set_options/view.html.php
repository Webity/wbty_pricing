	...
	protected function addToolbar()
	{
		...
		if (file_exists($formPath)) {
            if ($canDo->get('core.create')) {
				$bar->appendButton( 'Standard', 'new', 'New', 'pricing_set_option.add', false );
			   // JToolBarHelper::addNew('routine.add','JTOOLBAR_NEW');
		    }
		    if ($canDo->get('core.edit')) {
				$bar->appendButton( 'Standard', 'edit', 'Edit', 'pricing_set_option.edit', false );
				
				if (isset($this->items[0]->state)) {
					//$bar->appendButton( 'Standard', 'archive', 'Archive', 'pricing_set_options.archive', false );
				}
				if (isset($this->items[0]->checked_out)) {
					$bar->appendButton( 'Standard', 'checkin', 'Check In', 'pricing_set_options.checkin', false );
				}
		    }
			
			if ($canDo->get('core.edit.state')) {
				$bar->appendButton( 'Standard', 'trash', 'Trash', 'pricing_set_options.trash', false );
			}
