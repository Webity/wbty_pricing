	...
	protected function addToolbar()
	{
		...
		if (file_exists($formPath)) {
            if ($canDo->get('core.create')) {
				$bar->appendButton( 'Standard', 'new', 'New', 'option_item.add', false );
			   // JToolBarHelper::addNew('routine.add','JTOOLBAR_NEW');
		    }
		    if ($canDo->get('core.edit')) {
				$bar->appendButton( 'Standard', 'edit', 'Edit', 'option_item.edit', false );
				
				if (isset($this->items[0]->state)) {
					//$bar->appendButton( 'Standard', 'archive', 'Archive', 'option_items.archive', false );
				}
				if (isset($this->items[0]->checked_out)) {
					$bar->appendButton( 'Standard', 'checkin', 'Check In', 'option_items.checkin', false );
				}
		    }
			
			if ($canDo->get('core.edit.state')) {
				$bar->appendButton( 'Standard', 'trash', 'Trash', 'option_items.trash', false );
			}
