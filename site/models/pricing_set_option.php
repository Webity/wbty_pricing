...
class Wbty_pricingModelpricing_set_option extends WbtyModelAdmin
{
	...
	protected function populateState()
	{
		...
		$this->setState('pricing_set_option.id', $pk);
	...
	protected function saveRelational($table, $key, $array) {
		...
		$query = 'DELETE FROM #__wbty_pricing_'.$table.' WHERE pricing_set_options_id='.$this->table_id;
		$this->_db->setQuery($query)->query();
		
		if ($array) {
			foreach ($array as $id) {
				$query = 'INSERT INTO #__wbty_pricing_'.$table.' SET '.$key.'='.$id.', pricing_set_options_id='.$this->table_id;
				$this->_db->setQuery($query)->query();
			}
		}
	...
	public function getRelational($table, $key, $id) {
		$query = 'SELECT '.$key.' FROM #__wbty_pricing_'.$table.' WHERE pricing_set_options_id='.$id;
	...
	protected function prep_link($data, $table, $prefix='#__wbty_pricing_') {
		...
		$this->_db->setQuery('SELECT * FROM '.$prefix.$table.' WHERE pricing_set_option_id='.$this->table_id.'');
	...
	protected function link_insert($data, $table, $prefix) {
		...
		foreach ($data as $datum) {
			...
			if ($datum['id']) {
				$q = "UPDATE ".$prefix.$table." SET ".$value." WHERE id = '{$datum['id']}' AND pricing_set_option_id = '{$this->table_id}'";
			} else {
