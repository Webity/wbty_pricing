...
class Wbty_pricingModeloption_type extends WbtyModelAdmin
{
	...
	protected function populateState()
	{
		...
		$this->setState('option_type.id', $pk);
	...
	protected function saveRelational($table, $key, $array) {
		...
		$query = 'DELETE FROM #__wbty_pricing_'.$table.' WHERE option_types_id='.$this->table_id;
		$this->_db->setQuery($query)->query();
		
		if ($array) {
			foreach ($array as $id) {
				$query = 'INSERT INTO #__wbty_pricing_'.$table.' SET '.$key.'='.$id.', option_types_id='.$this->table_id;
				$this->_db->setQuery($query)->query();
			}
		}
	...
	public function getRelational($table, $key, $id) {
		$query = 'SELECT '.$key.' FROM #__wbty_pricing_'.$table.' WHERE option_types_id='.$id;
	...
	protected function prep_link($data, $table, $prefix='#__wbty_pricing_') {
		...
		$this->_db->setQuery('SELECT * FROM '.$prefix.$table.' WHERE option_type_id='.$this->table_id.'');
	...
	protected function link_insert($data, $table, $prefix) {
		...
		foreach ($data as $datum) {
			...
			if ($datum['id']) {
				$q = "UPDATE ".$prefix.$table." SET ".$value." WHERE id = '{$datum['id']}' AND option_type_id = '{$this->table_id}'";
			} else {
