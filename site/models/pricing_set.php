 ...
 * Wbty_pricing model.
 */
class Wbty_pricingModelpricing_set extends WbtyModelAdmin
{
	public function getTable($type = 'pricing_sets', $prefix = 'Wbty_pricingTable', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true, $control='jform')
	{
		...
		// Get the form.
		$form = $this->loadForm('com_wbty_pricing.pricing_set.'.$control, 'pricing_set', array('control' => $control, 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	public function getItems($parent_id, $parent_key) {
		...
		if (count($data)) {
			...
			foreach ($data as $key=>$d) {
				...
				$return[$key+1] = $this->getForm(array(), true, 'pricing_set['.$key.']');
			}
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_wbty_pricing.edit.pricing_set.data', array());
	...
	protected function populateState()
	{
		...
		$this->setState('pricing_set.id', $pk);
	 ...
	 public function getItem($pk = null)
	{
		if ($item = parent::getItem($pk)) {
			...
			$item->options = $db->setQuery($query)->loadObjectList('id');
			
			foreach ( $item->options as $key => $values) {
				$query->select('*');
				$query->from('#__wbty_pricing_options as a');
				$query->join('#__wbty_pricing_option_items as oi ON oi.option_id = a.id');
				$query->where('a.id = '.(int)$key);
	...
	public function save($data) {
		...
		$pricing_set_option = JRequest::getVar('pricing_set_option', array(), 'post', 'ARRAY');
		$this->save_sub('pricing_set_option', $pricing_set_option, 'pricing_set_id');
	...
	protected function saveRelational($table, $key, $array) {
		...
		$query = 'DELETE FROM #__wbty_pricing_'.$table.' WHERE pricing_sets_id='.$this->table_id;
		$this->_db->setQuery($query)->query();
		
		if ($array) {
			foreach ($array as $id) {
				$query = 'INSERT INTO #__wbty_pricing_'.$table.' SET '.$key.'='.$id.', pricing_sets_id='.$this->table_id;
				$this->_db->setQuery($query)->query();
			}
		}
	...
	public function getRelational($table, $key, $id) {
		$query = 'SELECT '.$key.' FROM #__wbty_pricing_'.$table.' WHERE pricing_sets_id='.$id;
	...
	protected function prep_link($data, $table, $prefix='#__wbty_pricing_') {
		...
		$this->_db->setQuery('SELECT * FROM '.$prefix.$table.' WHERE pricing_set_id='.$this->table_id.'');
	...
	protected function link_insert($data, $table, $prefix) {
		...
		foreach ($data as $datum) {
			...
			if ($datum['id']) {
				$q = "UPDATE ".$prefix.$table." SET ".$value." WHERE id = '{$datum['id']}' AND pricing_set_id = '{$this->table_id}'";
			} else {
