 ...
 * Wbty_pricing model.
 */
class Wbty_pricingModeloption_item extends WbtyModelAdmin
{
	public function getTable($type = 'option_items', $prefix = 'Wbty_pricingTable', $config = array())
	{
	...
	public function getForm($data = array(), $loadData = true, $control='jform')
	{
		...
		// Get the form.
		$form = $this->loadForm('com_wbty_pricing.option_item.'.$control, 'option_item', array('control' => $control, 'load_data' => $loadData));
		if (empty($form)) {
			return false;
		}
	...
	public function getItems($parent_id, $parent_key) {
		...
		if (count($data)) {
			...
			foreach ($data as $key=>$d) {
				...
				$return[$key+1] = $this->getForm(array(), true, 'option_item['.$key.']');
			}
		}
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_wbty_pricing.edit.option_item.data', array());
	...
	protected function populateState()
	{
		...
		$this->setState('option_item.id', $pk);
	...
	protected function prepareTable(&$table)
	{
		...
		if (JRequest::getVar('option_id')) {
	$table->option_id=JRequest::getVar('option_id');
}
	...
	protected function saveRelational($table, $key, $array) {
		...
		$query = 'DELETE FROM #__wbty_pricing_'.$table.' WHERE option_items_id='.$this->table_id;
		$this->_db->setQuery($query)->query();
		
		if ($array) {
			foreach ($array as $id) {
				$query = 'INSERT INTO #__wbty_pricing_'.$table.' SET '.$key.'='.$id.', option_items_id='.$this->table_id;
				$this->_db->setQuery($query)->query();
			}
		}
	...
	public function getRelational($table, $key, $id) {
		$query = 'SELECT '.$key.' FROM #__wbty_pricing_'.$table.' WHERE option_items_id='.$id;
	...
	protected function prep_link($data, $table, $prefix='#__wbty_pricing_') {
		...
		$this->_db->setQuery('SELECT * FROM '.$prefix.$table.' WHERE option_item_id='.$this->table_id.'');
	...
	protected function link_insert($data, $table, $prefix) {
		...
		foreach ($data as $datum) {
			...
			if ($datum['id']) {
				$q = "UPDATE ".$prefix.$table." SET ".$value." WHERE id = '{$datum['id']}' AND option_item_id = '{$this->table_id}'";
			} else {
